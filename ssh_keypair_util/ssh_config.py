...
class AugeasSSHConfig(object):
    def __init__(self, username):
        ...
        if os.path.isfile(abs_ssh_config):
    ...
    def set_config_path(self, *args):
        LOG.debug("set_config_path({}) = '{}'".format(", ".join(args[:-1]), args[-1]))
    ...
    def save(self):
        ...
        self.augeas.save()
        # Chmod the config file - may be newly created.
        if os.path.isfile(self.ssh_config):
            os.chmod(self.ssh_config, (stat.S_IRUSR | stat.S_IWUSR) & stat.S_IRWXU)
